...
// Base
import "./openzeppelin-presets/finance/TempPaymentSplitter.sol";
    ...
    TempPaymentSplitter
{
    ...
    mapping(IERC20Upgradeable=>(address=>uint256)) tokenBalance;
    ...
    function _release(address payable account) internal returns (uint256) {
        ...
        if (feeRecipient != address(0) && feeBps > 0) {
            ...
            etherBalance[feeRecipient] += fee; 
        }

        // AddressUpgradeable.sendValue(account, payment - fee);
        etherBalance[account] += payment - fee;
    ...
    function _release(IERC20Upgradeable token, address account) internal returns (uint256) {
        ...
        if (feeRecipient != address(0) && feeBps > 0) {
            ...
            tokenBalance[token][feeRecipient] += fee;
        }

        // SafeERC20Upgradeable.safeTransfer(token, account, payment - fee);
        tokenBalance[token][account] += payment - fee;
    ...
    function withdrawEther(address payable account) external {
        uint256 balance = etherBalance[account];
        etherBalance[account] = 0;
        AddressUpgradeable.sendValue(account, balance);
    }

    function withdrawTokens(IERC20Upgradeable token, address account) external {
        uint256 balance = tokenBalance[token][account];
        tokenBalance[token][account] = 0;
        SafeERC20Upgradeable.safeTransfer(token, account, balance);
    }

    function updatePayees(
        address[] memory _newPayees, 
        ...
        reset(_newPayees, _newShares, _tokens);
    }
