...
contract PaymentSplitterUpgradeable is Initializable, ContextUpgradeable {
    ...
    function reset(
        ...
        require(_newPayees.length == _newShares.length, "PaymentSplitter: payees and shares length mismatch");
        require(_newPayees.length > 0, "PaymentSplitter: no payees");

        address[] memory currentPayees = _payees;
        for(uint256 i = 0; i < _tokens.length; i++) {
            for(uint256 j = 0; j < currentPayees.length; j++) {
                _shares[currentPayees[j]] = 0;
                _released[currentPayees[j]] = 0;
                _erc20Released[_tokens[i]][currentPayees[j]] = 0;
            }
            _erc20TotalReleased[_tokens[i]] = 0;
        }
        _totalShares = 0;
        _totalReleased = 0;
        delete _payees;

        for (uint256 i = 0; i < _newPayees.length; i++) {
            _addPayee(_newPayees[i], _newShares[i]);
        }
    }
}
